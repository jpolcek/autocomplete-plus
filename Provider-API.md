...
* `providerblacklist` (optional): Defines an associative array of provider key to scope selector(s) for which the provider(s) should not be used - this currently only supports a `fuzzyprovider` key
* `key` (optional): An identifier that can be used by other providers to suppress your provider from being used for scope selector(s)
